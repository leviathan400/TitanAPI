#pragma once
// op2_log.hpp — tiny append-only debug logger for the Layer1 mission.
//
// Writes to  <Outpost2.exe dir>\OPU\logs\cLayer1.log  (or <dir>\logs if the exe already sits in an OPU
...
inline void resolve_path() {
  ...
  if (n == 0 || n >= sizeof(exe)) { std::strcpy(g_path, "cLayer1.log"); return; }

  int slash = -1;                                // strip the exe filename -> directory
  for (int i = 0; exe[i]; ++i) if (exe[i] == '\\') slash = i;
  if (slash < 0) { std::strcpy(g_path, "cLayer1.log"); return; }
  exe[slash] = 0;
  ...
  ensure_dirs(dir);
  std::snprintf(g_path, sizeof(g_path), "%s\\cLayer1.log", dir);
}
